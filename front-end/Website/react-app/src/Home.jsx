  ...
  const SidebarContent = ({ onClose, isMobile }) => {
      ...
      if (storedUser) {
        try {
          ...
          if (parsedUser.user_id) {
            const cachedImage = sessionStorage.getItem(`profileImage_${parsedUser.user_id}`);
    ...
    useEffect(() => {
      const loadUserData = async () => {
        ...
        if (!token) {
          ...
          profileImageLoadedRef.current = false;
          return;
        }
        ...
        try {
          ...
          if (email) {
            ...
            if (data) {
              ...
              // Always load profile picture on mount
              if (data.user_id) {
      ...
      // Always load user data on mount/refresh
      loadUserData();
      ...
      const handleUserUpdate = () => {
        ...
        if (updated) {
          try {
            const parsedUser = JSON.parse(updated);
      ...
      // Listen for profile picture updates (forced reload)
      const handleProfilePictureUpdate = (event) => {
        const userId = event.detail?.userId || userData?.user_id;
        if (userId) {
          console.log('🔄 Force reloading profile picture...');
          profileImageLoadedRef.current = false;
          sessionStorage.removeItem(`profileImage_${userId}`);
          setProfileImage(null);
          setTimeout(() => loadProfilePicture(userId, true), 100);
        }
      };
    ...
    // Cleanup for profile image URLs to prevent memory leaks
    useEffect(() => {
      return () => {
        if (profileImage) {
    ...
    const loadProfilePicture = (userId, force = false) => {
      // Check sessionStorage cache first
      const cachedImage = sessionStorage.getItem(`profileImage_${userId}`);
      if (cachedImage && !force) {
        console.log('📸 Loading profile picture from cache');
        setProfileImage(cachedImage);
        profileImageLoadedRef.current = true;
        return;
      }

      // Check if we already loaded it (unless forcing reload)
      if (profileImageLoadedRef.current && !force) {
        console.log('📸 Profile picture already loaded (cached)');
        return;
      }
        ...
        .then((blob) => {
          if (blob) {
            ...
            reader.onloadend = () => {
              ...
              profileImageLoadedRef.current = true;
              console.log('✅ Profile picture loaded and cached');
            };
            reader.onerror = () => {
    ...
    const getActive = () => {
      ...
      const mapping = [
        ...
        { key: "careerbot", match: "/careerbot" },
        { key: "resumes", match: "/resumes" },
        { key: "jobs", match: "/jobs" },
        { key: "applications", match: "/applications" },
      ];
      const found = mapping.find((m) => path.includes(m.match));
    ...
    const handleLogout = () => {
      ...
      if (userData?.user_id) {
        sessionStorage.removeItem(`profileImage_${userData.user_id}`);
      }
